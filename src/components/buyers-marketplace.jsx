...
export function BuyersMarketplace({ cart: initialCart, setCart: updateCart }) {
  ...
  const addToCart = (productId) => {
    ...
    setCart(newCart);
    if (updateCart) updateCart(newCart);
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-blue-600 text-white p-4 rounded-lg mb-8 flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="font-medium">
                {getTotalCartItems()} item(s) in cart - ₦{getCartTotal().toFixed(2)}
              </span>
            </div>
            <Button 
              variant="secondary" 
        ...
        <Dialog open={showCart} onOpenChange={setShowCart}>
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="text-blue-800">Shopping Cart</DialogTitle>
              <DialogDescription>
            ...
            <div className="space-y-4">
              {getCartProducts().length === 0 ? (
                <div className="text-center py-8">
                  <ShoppingCart size={48} className="mx-auto text-blue-300 mb-4" />
                  <p className="text-blue-600">Your cart is empty</p>
                </div>
              ) : (
                <>
                  {getCartProducts().map((product) => (
                    <div key={product.id} className="flex items-center space-x-4 p-4 bg-blue-50 rounded-lg">
                      <ImageWithFallback
                        src={product.image}
                        alt={product.name}
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-blue-600 text-sm">{product.farmer}</p>
                        <p className="text-blue-800 font-medium">₦{product.price} per {product.unit}</p>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Button
                          size="sm"
                        ...
                        <span className="w-12 text-center font-medium text-blue-800">
